...
BEGIN
   ...
   -----------------------------------------------------------------
   BEGIN
      -- 2026-07-03: DEFAULT assignment eliminates the inferred next_state LATCH.
      -- Some CASE/if branches did not assign next_state -> Quartus inferred a latch
      -- and clocked it off whatever 1-bit flag was in cpu_rdy -> the 2-try-boot
      -- flakiness (BUG2, see r65c02-latch-loop note).  Defaulting to "stay in the
      -- current state" matches the latch's usual behaviour (after current_state
      -- advances, the held next_state equals the new current_state) and makes
      -- next_state purely combinational (no latch) in ALL paths.
   ...
   BEGIN
      ...
      CASE current_state IS
         ...
         WHEN s268 => 
            ...
            END IF;
         WHEN s305 =>
            -- jyv 2026-07-03 (BUG3): combinational-loop fix.  The original
            -- four branches assigned IDENTICAL offset_o/ld_o/ld_pc_o; only
            -- sig_SYNC actually depends on the page-cross compare.  With the
            -- assignments under the compare, the ld_pc_o/offset_o logic cones
            -- included adr_nxt_pc_i, closing the 94-node combinational loop
            -- ld_pc_o -> reg_pc ci_o_i (cout AND ld_pc) -> PC adder ->
            -- adr_nxt_pc_i -> output_proc -> ld_pc_o (Quartus Critical
            -- Warning 332081, "estimating delays through the loop" =
            -- untrustworthy timing on silicon).  Hoisting the invariant
            -- assignments out of the compare is FUNCTIONALLY IDENTICAL and
            -- breaks the loop: the compare now feeds only sig_SYNC here
            -- (registered into sync_o_cld), plus next_state / clocked
            -- captures elsewhere -- all of which end at flip-flops.
            IF (rdy_i = '1') THEN
               ...
               ld_o <= "11";
               ld_pc_o <= '1';
               IF ((zw_b3 = adr_nxt_pc_i (15 downto 8)) AND (nmi_i = '1')) THEN
                  NULL;
               ELSIF ((zw_b3 = adr_nxt_pc_i (15 downto 8)) AND (irq_n_i = '0' and
                      reg_F(2) = '0')) THEN
                  NULL;
               ELSIF (zw_b3 = adr_nxt_pc_i (15 downto 8)) THEN
                  sig_SYNC <= '1';
               END IF;
            END IF;
         WHEN s306 =>
